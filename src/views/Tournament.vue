  ...
  <v-container fluid>
    <v-dialog v-model="dialog" persistent max-width="500px">
      <template v-slot:activator="{ on }">
        <v-btn color="blue" dark class="mb-2" v-on="on">Add Tournament</v-btn>
      </template>
      <v-card>
        <v-form ref="form" v-model="valid">
          ...
          <v-card-actions>
            ...
            <v-btn
              ...
              @click="addTournament"
            >Submit</v-btn>
            <v-btn color="blue" v-if="isUpdate === 1" @click="updateTournament">Update</v-btn>
            <v-btn color="blue" @click="resetTournamentDialog">Cancel</v-btn>
          </v-card-actions>
        </v-form>
      </v-card>
    </v-dialog>
    ...
    <v-dialog v-model="deleteDialog" max-width="290">
      <v-card class="text-xs-center dialog-delete">
        ...
        <v-card-actions>
          <v-btn color="blue darken-1" flat @click="deleteTournament">Yes</v-btn>
          <v-btn color="blue darken-1" flat @click="deleteDialog = false;deleteTournamentId = 0;">No</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>

    <v-dialog v-model="teamDialog" persistent scrollable max-width="450">
      <v-card>
        ...
        <v-card-text>
          ...
          <v-card-actions>
            ...
            <v-btn color="blue" @click="teamDialog=false;">Cancel</v-btn>
          </v-card-actions>
        </v-card-text>
      </v-card>
    </v-dialog>

    <v-dialog v-model="addTeamsDialog" persistent scrollable max-width="450">
      <v-card>
        <v-card-title>
          <span class="headline">{{ selectedTournament.tournamentName }}</span>
        </v-card-title>
        ...
        <v-card-actions style="justify-content: flex-end">
          <v-btn color="blue" :disabled="!selectedTeamsIdsToAdd.length" @click="addTeams">Add</v-btn>
          <v-btn
            color="blue"
    ...
    <v-data-table :headers="headers" :items="tournaments" class="elevation-5 tournamentTable">
      <template v-slot:items="props">
        ...
        <td class="text-xs-center">
          <v-btn color="blue" @click="showTeamDialog(props.item)">Teams</v-btn>
        </td>
        <td class="text-xs-center">
          <v-icon @click="updateTournamentDialog(props.item)">mdi-square-edit-outline</v-icon>
          <v-icon @click.stop="deleteTournamentDialog(props.item.id)">mdi-trash-can-outline</v-icon>
        </td>
      </template>
  ...
  methods: {
    ...
    updateTournament() {
      if (this.$refs.form.validate()) {
        ...
        let id = this.selectedTournament.id;
    ...
    deleteTournament() {
      let id = this.deleteTournamentId;
